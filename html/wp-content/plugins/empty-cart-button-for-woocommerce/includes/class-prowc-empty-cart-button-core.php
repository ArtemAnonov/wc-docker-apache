	...
	class ProWC_Empty_Cart_Button_Core {
		...
		function enqueue_scripts( $atts ) {
			if ( 'confirm_with_pop_up_box_jquery' == get_option( 'prowc_empty_cart_confirmation', 'no_confirmation' ) ) {
				...
				wp_enqueue_script(
					'prowc-empty-cart-button-confirm-lib',
				...
				wp_localize_script(
					'prowc-empty-cart-button-confirm',
					'prowc_ecb_confirm_object',
					array(
						'content' => get_option( 'prowc_empty_cart_confirmation_text', __( 'Are you sure?', ECB_TEXTDOMAIN ) ),
					)
				);
			}
		}
		...
		function get_empty_cart_form_shortcode( $atts ) {
			...
			return $this->get_empty_cart_form( $atts['html_template'], $atts['html_style'], $atts['html_class'], apply_filters( 'prowc_empty_cart_button', 'Очистить корзину', 'value_text' ) );
		}
		...
		function output_empty_cart_form() {
			echo $this->get_empty_cart_form(
				...
				apply_filters( 'prowc_empty_cart_button', 'Empty cart', 'value_text' )
			);
		}
		...
		function get_empty_cart_form( $html_template, $html_style, $html_class, $label ) {
			...
			' onclick="return confirm(\'' . get_option( 'prowc_empty_cart_confirmation_text', __( 'Are you sure?', ECB_TEXTDOMAIN ) ) . '\')"' : '';
			$confirmation_class = ( 'confirm_with_pop_up_box_jquery' == $confirmation_type ) ?
			'prowc-empty-cart-button-confirm ' : '';
			switch ( get_option( 'prowc_empty_cart_button_tag', 'a' ) ) {
				case 'a':
					$button_form_html = '<a href="' . add_query_arg( array( 'prowc_empty_cart' => '' ), wc_get_cart_url() ) . '" style="' . $html_style . '"' .
